...
import { isSpecialCpf } from "@/lib/cpf";
...
export function BeneficiaryForm() {
  ...
  return (
    <form onSubmit={submit} className="space-y-4 rounded-lg border bg-white p-6 shadow-sm">
      <Field label="CPF" hint={isSpecialCpf(cpf) ? "Prefixo especial — bypass documental (REQ-BEN-006)" : "11 dígitos"}>
        <input value={cpf} onChange={(e) => setCpf(e.target.value)} required
               className="w-full rounded border px-3 py-2" placeholder="00000000000" />
      </Field>
      <Field label="Nome">
